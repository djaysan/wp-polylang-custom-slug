...
/**
 * WP_Polylang_Custom_Slug_Post class.
 */
class WP_Polylang_Custom_Slug_Post 
{
	/**
	...
	{
		add_filter( 'wp_unique_post_slug', [$this, 'wp_polylang_custom_slug_unique_slug_in_language'], 10, 6 );

		add_filter( 'query', [$this, 'wp_polylang_custom_slug_filter_queries'] );

		add_filter( 'posts_where', [$this, 'wp_polylang_custom_slug_posts_where_filter'], 10, 2 );

		add_filter( 'posts_join', [$this, 'wp_polylang_custom_slug_posts_join_filter'], 10, 2 );
	}

	public function wp_polylang_custom_slug_unique_slug_in_language( $slug, $post_ID, $post_status, $post_type, $post_parent, $original_slug )
	{
		...
		$join_clause  = $this->wp_polylang_custom_slug_model_post_join_clause();
		// " AND pll_tr.term_taxonomy_id IN (" . implode(',', $languages) . ")".
		$where_clause = $this->wp_polylang_custom_slug_model_post_where_clause( $lang );
	 ...
	 */
	public function wp_polylang_custom_slug_filter_queries( $query ) 
	{
		global $wpdb;
		...
		$is_pages_sql = preg_match(
			...
			$this->wp_polylang_custom_slug_standardize_query( $query ),
			$matches
		);
		...
		// Check if should contine. Don't add $query wp_polylang_custom_slug_should_run() as $query is a SQL query.
		if ( ! $this->wp_polylang_custom_slug_should_run() ) {
			return $query;
		}
		 ...
		 */
		$query = apply_filters( 'wp_polylang_custom_slug_sql_query', $sql_query, $matches, $join_clause, $where_clause );
	 ...
	 */
	public function wp_polylang_custom_slug_posts_where_filter( $where, $query ) {
		// Check if should contine.
		if ( ! $this->wp_polylang_custom_slug_should_run( $query ) ) {
			return $where;
		}
		...
		// " AND pll_tr.term_taxonomy_id IN (" . implode(',', $languages) . ")"
		$where .= $this->wp_polylang_custom_slug_model_post_where_clause( $lang  );
	 ...
	 */
	public function wp_polylang_custom_slug_posts_join_filter( $join, $query ) {
		...
		// " INNER JOIN $wpdb->term_relationships AS pll_tr ON pll_tr.object_id = ID".
		$join .= $this->wp_polylang_custom_slug_model_post_join_clause();
	 ...
	 * @return bool
	 */
	public function wp_polylang_custom_slug_should_run( $query = '' ) {
		...
		$disable = apply_filters( 'wp_polylang_custom_slug_disable', false, $query );
	 ...
	 */
	public function wp_polylang_custom_slug_standardize_query( $query ) {
	 ...
	 * @return string
	 */
	public function wp_polylang_custom_slug_model_post_join_clause() {
	 ...
	 * @return string
	 */
	public function wp_polylang_custom_slug_model_post_where_clause( $lang = '' ) {
		if ( function_exists( 'PLL' ) ) {
...
new WP_Polylang_Custom_Slug_Post();